...
ED = angular_spectrum(EO,-z,Dm,Dn,lambda,true); %initial phase at object, discard amplitude
phi = angle(ED);
% phi = SLMphasetiling( crop( phi ,Nslm,Mslm) ,NpixelX,NpixelY); % tiling & set outside of SLM to zero
phi = crop( phi ,Nslm,Mslm); % set outside of SLM to zero

%% optimization
ED = crop( exp(1i*phi) ,N,M);
...
aa = 1; %initial step
for iter=1:100
%     grad = real( -1i.* crop( exp(-1i*phi) ,N,M) .* angular_spectrum(4*EO.*error,-z,Dm,Dn,lambda,true) ); %backpropagation
    ...
    phi = phi - aa*grad;
%     phi = SLMphasetiling( crop( phi ,Nslm,Mslm) ,NpixelX,NpixelY); % tiling & set outside of SLM to zero
    phi = crop( phi ,Nslm,Mslm); % tiling & set outside of SLM to zero
    aa = aa*0.99;
    ED = crop( exp(1i*phi) ,N,M);
    EO = angular_spectrum(ED,z,Dm,Dn,lambda,true); %resulting wavefield at object
    EO = EO/max(abs(EO(:)));
...
saveFPImage(ED,[path,imgname,'_Wirtingerfullres',parameters,'.fp.img']);