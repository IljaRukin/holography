...
note that the phase for hologram display can either be directly displayed using "..." or saved in an image specific to the currently used SLM (with phase stored as bit values 0-255 in RGB colors) with "...".
...
- for easier handling and displaying the holograms from images on the fly (e.g. using irfanview) use the script "./pf2png" to convert ".fp.img" wavefield files to ".png" images where the phase at the SLMs is color coded in RGB using 256bits.
