function E = sommerfeld(U,zSlices,Dn,Dm,lambda,zeropad,antialias)
...
for iSlice = 1:nSlices
    ...
    r = sqrt( z^2 + coord2 );
    %spherical = (1/r) * exp(1i*2*pi/lambda*sign(z)*r) .*mask; %einfache Form (Punktlichtquelle --> Huygens)
    spherical = -(z./(2*pi*r.^2)).*exp(1i*2*pi/lambda*sign(z)*r).*(1i*2*pi/lambda-(1./r)) .*mask; %exakte Rayleigh-Sommerfeld Lösung
